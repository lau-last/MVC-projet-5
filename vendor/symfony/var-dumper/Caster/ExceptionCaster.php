        ...
        for ($j += $trace->numberingOffset - $i++; isset($frames[$i]); ++$i, --$j) {
            ...
            if (isset($f[$prefix.'src'])) {
                ...
                if (isset($label->attr['ellipsis'])) {
                    ...
                    $label = ExceptionCaster . phpsubstr_replace($prefix, "ellipsis-type=class&ellipsis={$label->attr['ellipsis']}&ellipsis-tail=1&title=Stack level $j.", 2, 0) . $label->value .'()';
                } else {
                    $label = ExceptionCaster . phpsubstr_replace($prefix, "title=Stack level $j.", 2, 0) . $label->value .'()';
                }
            } else {
                $label = ExceptionCaster . phpsubstr_replace($prefix, "title=Stack level $j.", 2, 0) . $lastCall;
            }
